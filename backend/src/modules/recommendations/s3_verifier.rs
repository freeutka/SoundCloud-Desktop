...
impl S3VerifierService {
    pub fn new(http: Client, storage_url: String, pg: PgPool) -> Arc<Self> {
        ...
        Arc::new(Self { http, storage_url: trimmed, pg })
    }
    ...
    pub async fn find_missing(&self, sc_track_ids: &[String]) -> AppResult<HashSet<String>> {
        ...
        .fetch_all(&self.pg)
        .await?;
        let mut by_id: std::collections::HashMap<String, (Option<DateTime<Utc>>, Option<DateTime<Utc>>)> =
        ...
        if !miss_ids.is_empty() {
            ...
            .execute(&self.pg)
            .await?;
            debug!(misses = miss_ids.len(), oks = ok_ids.len(), "S3 verify result");
        }
        Ok(missing)
    }
