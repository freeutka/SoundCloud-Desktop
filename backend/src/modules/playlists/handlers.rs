...
async fn search(
    ...
    let mut extra: Vec<(String, String)> = vec![(
        "access".into(),
        q.access.unwrap_or_else(|| "playable,preview,blocked".into()),
    )];
    if let Some(v) = q.q { extra.push(("q".into(), v)); }
    if let Some(v) = q.show_tracks { extra.push(("show_tracks".into(), v)); }
    Ok(Json(st.playlists.search(&ctx.access_token, page, limit, extra).await?))
}
...
async fn get_by_id(
    ...
    if let Some(v) = q.secret_token { params.push(("secret_token".into(), v)); }
    if let Some(v) = q.show_tracks { params.push(("show_tracks".into(), v)); }
    let url = build_url(&format!("/playlists/{playlist_urn}"), &params);
    let cache_key = format!("playlist-detail:{playlist_urn}");
    cached_or_fetch(&st, "GET", &url, CacheScope::Shared, None, 3600, Some(&cache_key), || async {
        st.playlists.get_by_id(&ctx.access_token, &playlist_urn, &params).await
    })
    .await
}
...
async fn get_tracks(
    ...
    if let Some(v) = q.secret_token { extra.push(("secret_token".into(), v)); }
    Ok(Json(st.playlists.get_tracks(&ctx.access_token, &playlist_urn, page, limit, extra).await?))
}
...
async fn get_reposters(
    ...
    let (page, limit) = p.resolved();
    Ok(Json(st.playlists.get_reposters(&ctx.access_token, &playlist_urn, page, limit).await?))
}
